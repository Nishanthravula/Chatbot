...
default_msg = 'Chat bot is still learning!!'
topic_mapping = {
    "politics": "Politics",
...
boosting_words = pd.read_csv('keywords.csv')
...
def get_top_5_topic_query_matches(topic, query):
    ...
    filter_query = urllib.parse.urlencode({"fq": "topic:\"%s\"" % topic})
    solr_url = 'http://35.237.47.57:8983/solr/' + reddit_ir_model + '/select?' + encoded_solr_query + \
               ...
               facet_query
    solr_url = 'http://35.237.47.57:8983/solr/' + reddit_ir_model + '/select?' + encoded_solr_query + \
               '&fl=id%2Cscore&wt=json&indent=true&rows=5&' + \
               filter_query
    print(solr_url)
...
def get_correct_topic_mapping(topic):
    return topic_mapping[str.lower(topic)]


def topic_query(topic, query):
    topic = get_correct_topic_mapping(topic)
...
def query_pre_processor(query):
    words_list = query.split()[:50]
    for i in len(words_list):
        if words_list[i] in boosting_words['text']:
            words_list[i] += "^2"
...
def execute_query():
    ...
    if not topic_name or str.lower(topic_name) == 'chitchat':
        return chit_chat_query(query)
    elif str.lower(topic_name) == 'all':
